...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 bg-zinc-950 border-b border-zinc-800 shadow-2xl shadow-black/80">
      ...
      <div className="bg-zinc-900/90 border-b border-zinc-800/80 px-4 py-1.5 text-xs text-zinc-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5" ref={navContainerRef}>
        <div className="flex items-center justify-between gap-4">
          ...
          <div 
            ...
            id="brand-logo-btn"
          >
            <div className="relative w-9 h-9 rounded-xl bg-zinc-900 border border-emerald-500/40 p-2 flex items-center justify-center shadow-lg group-hover:border-emerald-400 transition-all">
              <span className="font-black text-base text-emerald-400 font-mono tracking-tighter">B</span>
              <span className="font-black text-base text-zinc-100 font-mono tracking-tighter">W</span>
              <div className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-emerald-400 ring-2 ring-zinc-950 animate-ping" />
            </div>
            <div>
              <div className="flex items-center gap-1">
                <span className="text-base sm:text-lg font-black text-white tracking-wider font-mono">BLACKACC</span>
                <span className="text-base sm:text-lg font-black text-emerald-400 tracking-wider font-mono">WORLD</span>
              </div>
          ...
          <nav className="hidden lg:flex items-center gap-1.5">
            ...
            >
              ...
              >
                ...
                <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${activeDropdown === 'reviews' ? 'rotate-180 text-emerald-400' : 'text-zinc-500'}`} />
              </button>

              {/* Reviews Sub-Menu Grid (26 Services) */}
              {activeDropdown === 'reviews' && (
                <div 
                  className="absolute top-full left-0 mt-2 w-[620px] bg-zinc-950/98 border border-zinc-800 rounded-2xl shadow-2xl backdrop-blur-2xl p-5 z-50 animate-in fade-in slide-in-from-top-2 duration-150"
                  onMouseEnter={() => handleMouseEnter('reviews')}
                  onMouseLeave={handleMouseLeave}
                >
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 max-h-[380px] overflow-y-auto pr-1">
                    {reviewsServices.map(svc => (
                      <button
                        ...
                        className="text-left p-2 rounded-xl hover:bg-zinc-900 flex items-center justify-between group transition-colors"
                      >
                        <div className="flex items-center gap-2.5 min-w-0 pr-2">
                          <div className="w-6 h-6 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center p-1 shrink-0 group-hover:border-emerald-500/50">
                            <BrandIcon name={svc.iconKey} className="w-3.5 h-3.5" />
                          </div>
                          <span className="text-zinc-300 group-hover:text-white text-xs font-medium truncate">
                            {svc.title}
                          </span>
                        </div>
            ...
            >
              ...
              >
                ...
                <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${activeDropdown === 'bank_accounts' ? 'rotate-180 text-emerald-400' : 'text-zinc-500'}`} />
              </button>

              {/* Bank Accounts Sub-Menu Grid (8 Services) */}
              {activeDropdown === 'bank_accounts' && (
                <div 
                  className="absolute top-full left-0 mt-2 w-[480px] bg-zinc-950/98 border border-zinc-800 rounded-2xl shadow-2xl backdrop-blur-2xl p-5 z-50 animate-in fade-in slide-in-from-top-2 duration-150"
                  onMouseEnter={() => handleMouseEnter('bank_accounts')}
                  onMouseLeave={handleMouseLeave}
                >
                  <div className="flex items-center justify-between pb-3 border-b border-zinc-800 mb-3">
                    ...
                    {bankServices.map(svc => (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0 pr-2">
                          <div className="w-7 h-7 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center p-1.5 shrink-0 group-hover:border-sky-500/50">
                            <BrandIcon name={svc.iconKey} className="w-4 h-4" />
                          </div>
                          <div>
                            <span className="text-zinc-200 group-hover:text-white text-xs font-semibold block">
                              {svc.title}
                            </span>
            ...
            >
              ...
              >
                ...
                <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${activeDropdown === 'accounts' ? 'rotate-180 text-emerald-400' : 'text-zinc-500'}`} />
              </button>

              {/* Accounts Sub-Menu Grid (19 Services) */}
              {activeDropdown === 'accounts' && (
                <div 
                  className="absolute top-full right-0 lg:left-0 mt-2 w-[580px] bg-zinc-950/98 border border-zinc-800 rounded-2xl shadow-2xl backdrop-blur-2xl p-5 z-50 animate-in fade-in slide-in-from-top-2 duration-150"
                  onMouseEnter={() => handleMouseEnter('accounts')}
                  onMouseLeave={handleMouseLeave}
                >
                  <div className="flex items-center justify-between pb-3 border-b border-zinc-800 mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 max-h-[380px] overflow-y-auto pr-1">
                    {accountsServices.map(svc => (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0 pr-2">
                          <div className="w-6 h-6 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center p-1 shrink-0 group-hover:border-purple-500/50">
                            <BrandIcon name={svc.iconKey} className="w-3.5 h-3.5" />
                          </div>
                          <span className="text-zinc-300 group-hover:text-white text-xs font-medium truncate">
                            {svc.title}
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-3 py-2 rounded-xl text-xs font-medium text-sky-400 hover:text-sky-300 hover:bg-sky-500/10 transition-colors flex items-center gap-1.5"
            >
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              onClick={onOpenCart}
              className="relative flex items-center gap-2 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs sm:text-sm font-bold rounded-xl shadow-lg shadow-emerald-950/50 transition-transform active:scale-95"
            >
              <ShoppingCart className="w-4 h-4" />
              <span>Cart</span>
              {cartCount > 0 && (
                <span className="flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-black bg-zinc-950 text-emerald-400 rounded-full border border-emerald-400">
                  {cartCount}
                </span>
              )}
            </button>
